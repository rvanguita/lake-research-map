...
The following capabilities are implemented and covered by the current 447 passing tests plus two opt-in MySQL tests skipped in the default run (229 before this cycle, followed by regression, application-wide page, evidence-workflow, retrieval, provenance, semantic-persistence, MySQL migration, and year-bound coverage):
...
- **Deliverable:**
  ...
  - **Impact and citations:** tail and count-model diagnostics; Price's index, citation half-life and Sleeping Beauty coefficient (gated on `WP-23`).
...
- **Status:** Complete on 2026-09-23 -- all three verdicts of `audit citation-years` pass. *Trajectories*: 3,053 of 3,099 observed works (98.5%) have a known trajectory; 584 published before 2012 are left-censored, so longevity and Sleeping Beauty use only the 2,514 complete from publication. *Cited-reference years*: **93,858 of 101,716 references dated (92.3%)**, over one list per work -- 2,815 Crossref deposits, 49 OpenAlex lists where no deposit exists, 235 works with a corroborated empty list, 16 with no list from either provider. *Validation*: provider year within one year of the corpus's for 98.9% of works, and 6 of 93,858 dated references newer than their citer (99.99%). Crossref supplied the years without touching the OpenAlex quota: 385 requests, no throttling. Caveats stated rather than averaged away: IET deposits almost no references (16 across 84 works), so IET's reference years rest on the OpenAlex resolution still running; and the two providers agree on a work's reference *count* within 10% for only 50.3% of works, because Crossref deposits keep books and reports that OpenAlex does not resolve to works -- which is why each work uses one list and the lists are never spliced. See `docs/evidence/2026-09-23-wp23-and-review.md`.
...
- **Status:** Both completion gates **pass**: graph integrity (0 self-loops, 0 dangling edges, and 100% agreement between OpenAlex's `referenced_works` and `cites:` indexes over the 3,314 corpus-internal pairs that can be cross-checked) and access validation (all 50 articles the IEEE CSV licenses CC/OAPA are open in OpenAlex; the two sources name the same CC licence in 95.7% of 46 pairs). Backward coverage is **97.1%**, not the 100% first recorded: an OpenAlex zero is only a known empty list when no other source contradicts it, and for 89 of the 324 zero-reference works Crossref or the corpus metadata does -- OpenAlex's `reference_count` is `len(referenced_works)`, which is empty when OpenAlex lacks the list. Forward coverage is 875 of 3,099 (28.2%), so the disruption index stays unavailable, which the completion rule explicitly allows; the batched forward crawl (233 requests for the rest) runs in the next quota window, after which the gates are re-run over the whole graph and the package closes. OACA stays unavailable regardless: no confounder-controlled model exists.
