...
- **Status:** Advisory locking, persistent logs, signal handling, resumable embeddings, heartbeat recovery, Airflow retry/timeouts, local-only ports, and resource bounds delivered. Per-table role provisioning was removed on 2026-09-22 -- see Evidence -- so the package no longer waits on passwords that were never going to be set. Retry/timeout policy and abandoned-run recovery remain open.
